...
export default function LoginScreen() {
  ...
  // Подстраиваем высоту под мобильные WebView
  useEffect(() => {
    const resizeHandler = () => {
    ...
    >
      <div className="w-full sm:max-w-sm bg-[#1a2338] p-6 rounded-2xl shadow-md flex-shrink-0 pb-[calc(env(safe-area-inset-bottom)+80px)]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full bg-[#24304a] p-4 rounded-2xl text-base text-white placeholder-gray-400 focus:ring-2 focus:ring-accent outline-none transition"
            />
          ...
          <button
            type="submit"
            className="w-full bg-accent hover:bg-green-600 transition py-4 rounded-2xl font-bold text-white text-base"
          >
            Войти
          </button>
        </form>
        ...
        <p className="text-center mt-4 text-sm text-gray-400">
          Нет аккаунта?{' '}
          <a href="/register" className="text-accent hover:text-green-500 transition inline-block mt-1">
            Регистрация
          </a>
        </p>
      </div>
